...
//         setSectionTwo(sectionTwo.filter((ele: any) => ele.id !== id));
//     }
...
const SectionTwo = () => {
    ...
    const { data: { section_two } } = useContext(PrincipalContext);

    return (
        <div className='flex flex-col items-center'>
            <h2 className='font-bold text-2xl mb-5'>Estructura del Main</h2>
            <Container>
                <ShapeOne data={section_two?.main_one} />
                <ShapeTwo data={section_two?.main_two} />
                <ShapeThree />
                <ShapeFour data={section_two?.main_four} />
                <ShapeFive />
                <ShapeSix />
            </Container>
        </div>
